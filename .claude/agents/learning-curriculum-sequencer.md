...
| LOGISTIC | The information about the logistics of delivering the course, such as scheduling, location, resources and time available. |
...
4. For each `item_type: "lesson"`, set `style` to exactly one value from the `delivery_style` enum — `lecture`, `lecture_discussion`, `lecture_demo`, `group_discussion`, `hands_on_practical`, `briefing`, or `project_based`. Pick the one that matches how the item is actually taught (a front-of-class explanation is `lecture` or `lecture_discussion`; a hands-on exercise is `hands_on_practical`; a project-based activity is `project_based`; a short one-way announcement is `briefing`). Do not use free text here — only these tokens.
