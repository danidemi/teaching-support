...
| DESIGN | `<ROOT>/design/knowledge_goals_graph.md` | A graph representing what knowledge are needed to obtain other knowledge that in the end allow the student to obtain a skill. Written by `learning-curriculum-architect`: persona roster, `Baseline`/`DesiredResult`/`Prerequisite` nodes, `Enables` edges, deliberate roots, depth staging. |
