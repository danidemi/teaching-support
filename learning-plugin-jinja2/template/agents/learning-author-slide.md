...
{% block agent_name %}learning-author-slide{% endblock %}
...
- `material/slides/session-NN.yml` — every segment of that {{ stores.curriculum.name }} session
...
Read every session in {{ stores.curriculum.name }}, not only the one you were asked to cover.
...
- An item at an earlier session, or an earlier `sequence` within your own session, is **already
  ...
  re-explain it. Use the target node's `Requires` edges in {{ stores.design.name }} to recognize a
...
1. List the session's items from {{ stores.curriculum.name }} in `sequence` order.
2. Group them into segments: one coherent teaching arc per segment (typically a lesson item with
   ...
   their own {{ stores.curriculum.name }} items, not something this deck restages as slides. An
