...
1. Determine `kind` from what the orchestrating skill asked for (`assessment` for a `checkpoint`
   ...
   item).
2. For an `assessment`-kind quiz, derive every question from the {{ stores.curriculum.name }} item's `rubric`
   text — each pass/fail criterion should be checkable by at least one question. For
   `prereq-check`/`engagement`, derive questions from the target node's `description` in {{ stores.design.name }}.
