...
1. `{{ references.ssot_structure.path }}` — canonical store paths.
...
3. `{{ references.didactic_material_catalog.path }}` — the full type → trigger → path registry. This is
...
`{{ references.didactic_material_catalog.path }}`'s trigger column to build the list of
...
`{{ references.didactic_material_catalog.path }}`. 
