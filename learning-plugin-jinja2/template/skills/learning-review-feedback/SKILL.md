...
description: >-
  ...
  books) — it resolves which agent and spec own the file from `{{ references.didactic_material_catalog.path }}`,
...
1. `{{ references.didactic_material_catalog.path }}` — maps a material type to its
