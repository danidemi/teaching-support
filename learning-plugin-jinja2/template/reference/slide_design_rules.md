...
instructor redundant, which for an instructor-led course ({{ stores.logistics.name }}: human-taught, synchronous) is a
...
- Every slide traces to an objective id from `{{ stores.goals.path }}` — enforced as an error. A slide
