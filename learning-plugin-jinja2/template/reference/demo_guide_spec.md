...
Two AsciiDoc files per {{ stores.curriculum.name }} item whose `didactic_activity` is `demo`, both
...
`duration_minutes` in {{ stores.curriculum.name }}, with the reasoning behind any setup-time
