...
One AsciiDoc file per {{ stores.curriculum.name }} item whose `didactic_activity` is `demo`, at
...
is a **grounded guess**, not a tested fact, unless the {{ stores.curriculum.name }} item or a DESIGN
...
Why::
  ...
  Teaches: <which {{ stores.design.name }} node/objective this step advances>
...
{{ stores.curriculum.name }}. A mismatch is not corrected by silently rewriting the budget — record
