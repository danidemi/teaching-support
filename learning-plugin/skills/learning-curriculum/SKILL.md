...
2. Execute the `learning-design-reviewer` agent against the DESIGN store it just
   wrote. The reviewer only reports findings — it never edits DESIGN itself.
3. If the reviewer returns blocking findings (coalescence, unfounded references), hand its report
   back to `learning-curriculum-architect` and ask it to amend DESIGN — never
